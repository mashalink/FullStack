...
describe("POST /api/blogs/:id/comments", () => {
  test("a comment can be added to a blog", async () => {
    ...
    assert(response.body.comments);
    assert(response.body.comments.includes("nice post"));

    const blogsAtEnd = await helper.blogsInDb();
    const updatedBlog = blogsAtEnd.find((b) => b.id === blogToComment.id);

    assert(updatedBlog.comments.includes("nice post"));
  });

  test("adding an empty comment fails with 400", async () => {
    const blogsAtStart = await helper.blogsInDb();
    const blogToComment = blogsAtStart[0];
  ...
  test("adding comment to non-existing blog fails with 404", async () => {
