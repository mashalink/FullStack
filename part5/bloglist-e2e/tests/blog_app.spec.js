...
describe("Blog app", () => {
  ...
  describe("When logged in", () => {
    ...
    test("a new blog can be created", async ({ page }) => {
      ...
      const createdBlog = page
        .locator(".blog")
        .filter({ hasText: "Test Blog Title" })
        .first();
      await expect(createdBlog).toBeVisible();
      await expect(createdBlog.locator(".blogTitle")).toContainText(
        "Test Blog Title",
      );
      await expect(createdBlog.locator(".blogTitle")).toContainText(
        "Test Author",
    ...
    test("a blog can be liked", async ({ page }) => {
      ...
      await expect(blog.locator(".blogDetails")).toContainText(
        `likes ${before + 1}`,
      );
    });
    ...
    test("blogs are ordered by likes (most likes first)", async ({ page }) => {
      ...
      const titlesInDom = await page.$$eval(".blog", (nodes) =>
